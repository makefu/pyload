...
from module.plugins.internal.misc import exists, fsjoin, lock, threaded
...
class IRC(object):
    ...
    def is_bot_online(self, bot):
        ...
        while time.time()-start_time < 30:
            origin, command, args = self.get_irc_command()
            if command == '401' and args[1] == self.nick:  #: ERR_NOSUCHNICK
                self.plugin.log_debug(_("Bot %s is offline") % bot)
                return False

            elif command == "311" and args[0] == self.nick:  #: RPL_WHOISUSER
        ...
        if self.xdcc_request_time:
            ...
            while time.time() - start_time < 30:
                ...
                if origin and command and args \
                    ...
                    text = args[1]
                    self.plugin.log_debug(_("PrivMsg: <%s> %s") % (bot, text))

                    m = re.match(r'\x01DCC ACCEPT .*? %s (?P<RESUME_POS>\d+)\x01' % dcc_port, text)
                    if m:
                        self.plugin.log_debug(_("Bot %s acknowledged resume at position %s") % (bot, m.group('RESUME_POS')))
...
class XDCC(Hoster):
    ...
    def xdcc_send_resume(self, resume_position):
        if not self.config.get('try_resume'):
            return 0

        else:
            bot = self.info['pattern']['BOT']
            return self.irc_client.xdcc_request_resume(bot, self.dcc_port, self.dcc_file_name, resume_position)
    ...
    def proccess_irc_command(self, origin, command, args):
        ...
        text = args[1]
        self.log_debug(_("PrivMsg: <%s> %s") % (bot, text))
